	...
	paras.$win.scroll(function(){
		...
		if(paras.$winTop >= paras.mainTop && paras.$winTop < paras.mainTop +110){
			...
			paras.cNav($("#detail-nav a[href='#detail_para']"));
				...
				/// 闪购，抢
				var shanGouBuy = function(){
					$("#J_yuyue,#J_nav_yuyue").click(function(e){
						e.preventDefault();
						if($(this).hasClass('yuyue-haved')){
							return ;
						}
						Souche.MiniLogin.checkLogin(function(){
							...
							Bimu.ajax.submitpost(config.api_shanGou,config.api_jsonparams,function(data){
								$('body').append(data);
								$(".wrapGrayBg").show();
