...
(function(angular, analytics) {
    ...
    angulartics.waitForVendorApi = function(objectName, delay, containsField, registerFn) {
        ...
        if (!Object.prototype.hasOwnProperty.call(window, objectName) || (containsField !== undefined && window[objectName][containsField] === undefined)) {
            setTimeout(function() {
                angulartics.waitForVendorApi(objectName, delay, containsField, registerFn);
            }, delay);
        } else {
        ...
        .provider('$analytics', function() {
            ...
            var registerSetUserPropertiesOnce = function(fn) {
                ...
                angular.forEach(cache.setUserPropertiesOnce, function(properties, index) {
                    setTimeout(function() {
                        api.setUserPropertiesOnce(properties);
                    }, index * settings.pageTracking.bufferFlushDelay);
                });
            };
            ...
            if ($analytics.settings.pageTracking.autoTrackVirtualPages) {
                ...
                if ($injector.has('$route')) {
                    $rootScope.$on('$routeChangeSuccess', function(event, current) {
                        ...
                        if (s.pageName === "" || s.channel === "") return;
                        var url = $analytics.settings.pageTracking.basePath + $location.url();
                        $analytics.pageTrack(url);
                    });
                }
                if ($injector.has('$state')) {
                    $rootScope.$on('$stateChangeSuccess', function(event, current) {
                        var url = $analytics.settings.pageTracking.basePath + $location.url();
                        $analytics.pageTrack(url);
                        console.log('$stateChangeSuccess -> $analytics.pageTrack, url =', url);
                    });
                }
            }
        }
